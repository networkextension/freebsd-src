 ...
 * and Windows and other USB4NET hosts because they all speak the same Apple protocol.
 *
 * Cross-checked against the reference ThunderboltIP implementation (frame
 * transmit / poll / frame validation).  Data frames are little-endian raw bytes on
...
/* Validate a received frame header (ThunderboltIP frame validation). */
static bool
