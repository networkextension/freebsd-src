 ...
 * the Intel NHI ring engine (ring descriptor/options programming and ring
 * start) and the USB4 Spec 2.0 §8 control-frame framing (payload + CRC32C footer).
 *
...
/*
 * Program and enable the ring (the NHI ring-start sequence): write the descriptor base
...
nhi_ring_program(struct nhi_ring *r)
{
	...
	/*
	 * NHI ring start: frame-mode rings (ThunderboltIP data path) write max-frame
	...
	if (r->frame_mode) {
		...
		/*
		 * Step 2 (the NHI ring-start sequence: "Now that the ring valid bit
 ...
 * status clears itself (the Intel NHI interrupt auto-clear quirk).
 */
int
nhi_ring0_setup(struct nhi_softc *sc)
{
	...
	/*
	 * Intel auto-clear quirk ON (set for all
	...
	/*
	 * Program the interrupt throttle for vector 0 - BOTH the reference
	 * implementation (128us in 256ns units) and Apple (initNHI
 ...
 * the CRC32C footer (inverted, big-endian; per the USB4 Spec 2.0 §8 control-packet framing), set SOF=EOF=pdf, and
...
nhi_ctl_tx(struct nhi_softc *sc, uint8_t pdf, const void *data, u_int len)
{
	...
	/*
	 * Control frames are big-endian per dword on the wire (USB4 Spec 2.0
	 * §8 control-packet framing); the CRC32C is then computed over those bytes.
	 */
	for (i = 0; i < len; i += 4)
	...
	for (i = 0; ; i++) {
		...
		if (((d->attrs >> NHI_DESC_FLAGS_SHIFT) &
			...
			*pdf = (d->attrs >> NHI_DESC_EOF_SHIFT) & 0xf;
			 ...
			 * just consumed (the NHI ring descriptor re-post rule).  The
...
nhi_ring0_intr(struct nhi_softc *sc)
{
	 ...
	 *
	 * Storm-proof pattern (the NHI ring interrupt start-poll path): read
...
nhi_data_setup(struct nhi_softc *sc)
{
	...
	/* Frame mode (NHI frame-mode ring flag): RX filters ThunderboltIP data
	 * PDFs and runs E2E flow control fed by our TX ring (ThunderboltIP). */
	tx->frame_mode = true;
	tx->e2e = true;
	rx->frame_mode = true;
	...
	/* Prime the RX ring before the tunnel is enabled (the reference ordering). */
	bus_dmamap_sync(rx->buf_tag, rx->buf_map, BUS_DMASYNC_PREREAD);
	for (i = 0; i < rx->count - 1; i++)
		nhi_rx_desc_post(rx, i);
	rx->head = rx->count - 1;
