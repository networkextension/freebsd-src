 ...
 * (Linux drivers/thunderbolt/icm.c: icm_request wraps it as TB_CFG_PKG_ICM_CMD;
 ...
 * transport API (DESIGN.md §4.1).  Message layout cross-checked against Linux
 * tb_msgs.h (struct icm_pkg_header / icm_tr_pkg_driver_ready_response).
 */
...
{
	uint8_t req[36];	/* struct icm_tr_pkg_approve_xdomain (Linux tb_msgs.h) */
	uint8_t resp[NHI_CTL_FRAME_SIZE];
	uint8_t code, flags, pdf;
	u_int rlen;
	int error, attempt, i;
	...
	/*
	 * Linux icm_request: send the command, then match a reply by
