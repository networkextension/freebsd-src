 ...
 * Bridge).  Wire formats cross-checked against the USB4 Inter-Domain Service
 * Spec 2.0.
 *
 * Endianness: nhi_ctl_rx delivers payloads as native dwords and nhi_ctl_tx
 * byteswaps native->big-endian per dword (matching the USB4 Inter-Domain Service Spec 2.0 wire format).
    ...
    uint32_t value)
{
	 ...
	 * type in the top byte (USB4 Inter-Domain Service Spec 2.0).
	 */
...
nhi_xdomain_build_dir(struct nhi_softc *sc)
{
	 ...
	 * block.  The peer computes num_entries = length/4 (USB4 Inter-Domain Service Spec 2.0), so
...
nhi_xdomain_handle(struct nhi_softc *sc, const uint8_t *f, u_int len)
{
	 ...
	 * (per the USB4 Inter-Domain Service Spec 2.0: route = (route_hi<<32|route_lo) &
