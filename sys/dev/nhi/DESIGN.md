...
2. **G2 — Thunderbolt networking (`if_tbt`).** Implement the Apple *ThunderboltIP*
   (a.k.a. *thunderbolt-net* / USB4NET) protocol so that two TB-connected hosts each expose a standard
...
the canonical open implementation.)
...
- **ICM (Internal Connection Manager)** — firmware *inside* the controller manages the
  ...
  receives **notifications** on ring 0. This is what Windows and other modern host stacks use on
...
- **SW-CM (software CM)** — the OS owns the fabric: it walks routers by issuing config
  ...
  hotplug. This is the "Apple/`tb`" model and the full software connection manager. **Large, complex.**
...
> Gen 5 — see `TESTPLAN-thinkpad-t14s-gen5.md`) is expected to run **no ICM**: the reference
> open implementation drives those controllers with a *software* CM, not the ICM client. So D1's
...
> only *half* right. `fw_sts=0x80000121` has **bit0 `ICM_EN` set** — the reference
> implementation's ICM-firmware-running test — so ICM firmware **is present** on this integrated MTL
...
> the control-frame payload to big-endian (USB4 Spec 2.0 §8 control-packet framing), so every
...
> meaningless on integrated parts (the reference implementation has no opmode hook for them) — the DRIVER_READY
...
> "integrated controllers use ICM and it'll just work" (from the reference implementation's read
> of the ICM driver-ready path) is **not borne out on silicon** without first waking the CM. Next:
...
> frame matches the reference implementation's request byte-for-byte and the firmware is powered+ready, so the
> conclusion is that **this controller does not run a host-responsive ICM — it is in native
> USB4 mode.** On integrated parts the reference implementation reads no opmode (no opmode hook), so our `SAFE`
...
> router model, as in the reference implementation)** — what DESIGN.md filed as the Phase-5 SW-CM stretch. Force-power
...
service entry with the ThunderboltIP protocol UUID and `prtc*` keys. Compare to the USB4
Inter-Domain Service Spec 2.0 and the Apple ThunderboltIP protocol (the `APPLE_THUNDERBOLT_IP_PROTOCOL_UUID`).
...
from; the canonical open implementation and the USB4 spec are the
...
This mirrors the reference implementation's split (`nhi` ↔ `tb`/`xdomain` ↔ `thunderbolt-net`) but in newbus terms,
...
reference open implementation / spec section it was cross-checked against, so divergences are traceable.
...
- **R2 — ICM command/notification ABI is undocumented.** We reverse it from the reference
  open implementation + observed traffic; ABI varies by controller generation. *Mitigation:*
  start with the exact controller the reference implementation supports (Titan/Maple Ridge); pin device IDs.
...
- **R6 — RESOLVED: Meteor Lake ICM works; the earlier "dead end" was a driver byteswap bug.**
  ...
  (USB4 Spec 2.0 §8 control-packet framing), so the frame + CRC were malformed. With `nhi_ctl_tx`/`nhi_ctl_rx`
  ...
  the USB4 Spec 2.0 §8 control-packet framing before concluding a controller is unresponsive. `opmode=SAFE` is a non-signal on
  integrated parts (the reference implementation has no opmode hook); trust the DRIVER_READY response.
...
- The canonical open Thunderbolt/USB4 implementation — NHI transport, ICM client, XDomain,
  and the ThunderboltIP networking driver. Reference for ICM ABI, ring layout, property
  format, and the Apple protocol UUID/handshake.
