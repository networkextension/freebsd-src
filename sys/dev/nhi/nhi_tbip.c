 ...
 *
 * Cross-checked against the reference ThunderboltIP implementation.  Endianness: like
...
/*
 * Jenkins hash (jhash2) - used only to derive our synthetic
 * MAC so it is byte-identical to the one Apple ThunderboltIP (tbnet) would generate from the
 ...
 * so don't only react to the peer's LOGIN (the reference ThunderboltIP retries ~every 4.5 s).
...
nhi_tbip_start_login(struct nhi_softc *sc)
{
	if (!sc->has_peer || sc->paths_approved || sc->tbip_login_sent)
		return;
	/*
	 * Cap retries (ThunderboltIP TBNET_LOGIN_RETRIES = 60): a peer that went
