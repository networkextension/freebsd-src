 ...
 *
 * Per DESIGN.md: hardware facts here are cross-checked against the Intel NHI
 * register map (register offsets are hardware facts, not creative expression)
 * and the device IDs observed on the wire (SolidRun Honeycomb + GIGABYTE
 * GC-Titan Ridge JHL7540).  Items still tagged VERIFY are not yet confirmed on
 * silicon; confirm via probe before trusting.  If a probe disagrees, fix this
 * header (and DESIGN.md) first.
 *
 * Register offsets and bit positions in this revision were reconciled against
 * the Intel NHI register map (confirmed on silicon); the from-memory guesses
 * they replaced are noted in the commit message.
 */
#ifndef _NHI_REG_H_
#define _NHI_REG_H_
 ...
 * coherent DMA memory.  Layout of the Intel NHI ring descriptor.
 */
...
/*
 * Descriptor flag values (Intel NHI ring-descriptor flags) - they live in the
...
/*
 * BAR0 register map.  Confirmed offsets (the Intel NHI register map).
 */
...
/*
 * Per-ring control-block field offsets (the Intel NHI ring engine).  The
 ...
 * into the next ring's block (corrected and confirmed on Meteor
 * Lake silicon when ring 0 lands).
 */
 ...
 * land.)  Flags from the Intel NHI ring-flags field.
 */
 ...
 * (the NHI firmware operation-mode field).  CM_MODE means the internal connection manager
...
/*
 * NHI mailbox commands (the Intel NHI mailbox command set).  Issued via
 ...
 * capability registers, in PCI *config* space (the Intel NHI vendor-specific
 * capability registers).  The on-die Thunderbolt controller powers down until
 * the host asserts force-power; until then the connection-manager firmware
 * does not answer ring-0 ICM commands.  nhi_force_power() drives this.
 */
...
#define	 NHI_VS_CAP_22_DMA_DELAY_VAL	0x22		/* NHI force-power DMA delay */
...
/*
 * Ring interrupt registers (the Intel NHI ring engine).  Each is a bitmap with one bit per
...
/*
 * Ring-0 control-channel framing (USB4 Spec 2.0 §8).  A control
 ...
 * Values: the USB4 Spec 2.0 §8 control-packet types.
 */
...
/*
 * ICM messages (the ICM firmware protocol).  Every ICM packet opens
...
/* ICM notification (PDF ICM_EVENT) codes we care about (the ICM firmware protocol). */
...
/*
 * ICM commands for XDomain data paths (the ICM firmware protocol, TR variant for integrated
 ...
 * = 36 bytes (struct icm_tr_pkg_approve_xdomain, per the ICM firmware protocol); response echoes it.
 */
#define	NHI_ICM_APPROVE_XDOMAIN		0x10
#define	NHI_ICM_DISCONNECT_XDOMAIN	0x11

/*
 * ThunderboltIP (Apple) networking protocol (the Apple ThunderboltIP/USB4NET protocol).
...
/* network service property values (Apple ThunderboltIP). */
...
/*
 * ThunderboltIP data-frame header (the Apple ThunderboltIP frame format).  Rides
...
/* Per-DMA-frame SOF/EOF markers on the data ring descriptor (Apple ThunderboltIP). */
#define	NHI_TBIP_PDF_FRAME_START	1
#define	NHI_TBIP_PDF_FRAME_END		2

/*
 * XDomain discovery protocol (USB4 Inter-Domain Service Spec 2.0).
 ...
 * maps them to/from the wire (matches the USB4 Inter-Domain Service Spec 2.0
 * UUID big-endian dword encoding).
 *
...
/*
 * Property directory block (USB4 Inter-Domain Service Spec 2.0).  Built in
